...
public class MainGit {
	...
	final static Config CONF = ConfigFactory.load("carrot-config");

	private static final String LOCAL = //
	CONF.getString("carrot.local");

	private static final String REMOTE = //
	CONF.getString("carrot.remote");

	private static final int TIMEOUT = //
	CONF.getMilliseconds("carrot.timeout").intValue() / 1000;
	...
	static Git makeCloneBranch(final String branch) throws Exception {
		...
		final File local = new File(getLocal().getAbsolutePath() + "." + branch);
